...
def update_add(s, e, node, l, r, value):
    ...
    right = node * 2 + 1
    mid = (s + e) // 2
    update_add(s, mid, left, l, r, value)
    update_add(mid + 1, right, l, r, value)
...
def query_first(s, e, node, l, r):


def query_max(s, e, node, l, r, x):
...
def init(N, fatigue):
    # TODO: fatigue 는 길이 N 리스트(구간 i 의 초기 피로도).
    #       전역 자료 구조를 반드시 초기화할 것.
    global g_N, tree, arr, lazy_set, lazy_add
    g_N = N
    tree = [0] * (4 * N)
