...
def get_patterns(word):
    ...
    for i in range(len(word)):
        ...
        ret.append(masked_word)
    return ret


def activate(word):
    # 변환해서 patterns_dict 에 넣어주기

    if word_count[word] == 0:

        for pattern in get_patterns(word):
            patterns_dict[pattern].add(word)
    word_count[word] += 1
...
def deactivate(word):
    ...
    for pattern in get_patterns(word):
        patterns_dict[pattern].remove(word)

        if not patterns_dict[pattern]:
            del patterns_dict[pattern]
    del word_count[word]
    # 버켓 안에 있으면 제거
...
def top5Keyword(mRet: List[str]) -> int:
    ...
    for start in word_count:
        ...
        representative = None
        representative_cnt = -1

        while q:
            cur = q.popleft()

            cur_count = word_count[cur]

            total_count += cur_count

            if (
                    cur_count > representative_cnt
                    or
                    (
                            cur_count == representative_cnt
                            and cur < representative
                    )
            ):
                representative = cur
                representative_cnt = cur_count
            ...
            for pattern in get_patterns(cur):
                ...
                for nxt in patterns_dict[pattern]:
