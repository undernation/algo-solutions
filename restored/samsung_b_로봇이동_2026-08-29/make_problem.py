...
S.init(15)
S.build(1, 3, 3, 4, 4, 2, 0)
S.build(2, 8, 3, 4, 4, 2, 3)
p12, d12 = trace_path(1, 2)
FIG2 = svg_grid(1, 1, 13, 8, path=p12, cell=38,
                title="[Fig. 2] D1 -> D2 최단경로 (빨간 선, 16칸)")
...
# 첫 TC 명령 표는 fig_steps.py 가 만든 _tc1_table.md 를 그때그때 읽는다.
# (예전에는 표를 코드에 박아 두어 테케를 바꾸면 표가 옛 내용으로 남았다.)
examples_text = examples_text.replace(
    "[[TC1_TABLE]]", io.open("_tc1_table.md", encoding="utf-8").read().strip())
...
prob = {
    ...
    "constraints": [
        ...
        "5. 출입구는 교차로와 인접(대각 포함)한 자리에 배정되지 않으며, 이후 건물이 추가되어도 유지된다. 따라서 출입구는 건물의 모서리 칸에 오지 않는다.",
        "6. 경유 건물의 수 M은 최대 5이다.",
        "7. 도로는 방향성을 가지므로 출발지와 도착지를 뒤바꾸면 최단 거리가 달라질 수 있다.",
        "8. move() 에서 조건을 만족하는 경로가 존재하지 않는 경우는 주어지지 않는다.",
        "9. build() 는 한 테스트 케이스에서 최대 약 1,000회 호출되는 것으로 복원하였다.",
        "10. move() 는 한 테스트 케이스에서 최대 약 100회 호출되는 것으로 복원하였다.",
    ],
