...
#### 2026-08-21 (품)
풀었으나, 구역을 나누어서 관리하는 개념은 꼭 익혀두면 좋을것 같다.

아래와 같이 구역 관리 코드로 구현이 가능하다.

아래 코드에서, sy, sx 로 시작하는 부분은 이문제가 간단하게 흘린
호출 구역이 N / 10 이라는 내용을 통해서 구현할 수 있는 부분이다.
이걸 통해서 아래와 같이 세트 로다가 관리해주면 찾기도 매우쉽다.

파이썬에서는 set 에서 remove, add 시간복잡도가 해시 테이블 기반으로 구현되어있어
O(1) 이기 때문이다.
